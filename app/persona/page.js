    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-white dark:bg-gray-950">
      <div className="w-full max-w-4xl text-center">
        ...
        <p className="text-gray-500 mb-8 text-lg">
          Each persona has an identity and answers differently.
        </p>

        {/* map through predef personas */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {personas.map((persona) => (
            // nav to chat page with persona.id query
            <Link
              key={persona.id}
          ...
          >
            <div className="flex flex-col items-center text-center">
              {/* avatar */}
              <img
                src="https://api.dicebear.com/9.x/bottts/svg?seed=Random&backgroundColor=b6e3f4,c0aede,d1d4f9&radius=50"
                className="w-16 h-16 rounded-full mb-3 ring-4 shadow-md"
                alt="Avatar"
              />
